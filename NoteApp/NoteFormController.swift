...
class NoteFormController: UIViewController {
   ...
   private func dismissKeyboard() {
    ...
    @objc private func didTapSave() {
        ...
        if id != nil{
            NetworkManager.shared.updateNote(id: id!, parameters: parameters) {
                self.delegate?.updateTableView()
                self.dismiss(animated: true, completion: nil)
            }
        } else {
            if textField.text != "" && textView.text != "" {
                NetworkManager.shared.addNote(parameters: parameters) {
                    self.delegate?.updateTableView()
                    self.dismiss(animated: true, completion: nil)
                }
            } else {
                if textField.text?.isEmpty ?? true {
                    ...
                    DispatchQueue.main.async {
                        UIView.transition(with: self.textFieldCheckLabel, duration: 0.6, options: .showHideTransitionViews, animations: {
                ...
                if textView.text?.isEmpty ?? true {
                    ...
                    DispatchQueue.main.async {
                        UIView.transition(with: self.textViewCheckLabel, duration: 0.6, options: .transitionCrossDissolve, animations: {
